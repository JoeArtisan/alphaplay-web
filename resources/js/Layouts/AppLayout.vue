        ...
        <div class="min-h-screen bg-gray-100">
            <nav class="bg-white border-b border-gray-100">
                    ...
                    <div class="flex justify-between h-16">
                        <div class="flex">
                            ...
                            <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                                ...
                                <jet-nav-link :href="route('profile.show')" :active="route().current('profile.show')" v-if="$page.props.user">
                                    Cuenta
                                </jet-nav-link>
                                <jet-nav-link :href="route('welcome')" :active="route().current('welcome')" v-else>
                                    Inicio
                                </jet-nav-link>
                            </div>
                        </div>
                ...
                <div :class="{'block': showingNavigationDropdown, 'hidden': ! showingNavigationDropdown}" class="sm:hidden">
                    <div class="pt-2 pb-3 space-y-1">
                        <jet-responsive-nav-link :href="route('dashboard')" :active="route().current('dashboard')" v-if="$page.props.user">
                            Dashboard
                        </jet-responsive-nav-link>
                        <jet-responsive-nav-link :href="route('profile.show')" :active="route().current('profile.show')" v-if="$page.props.user">
                            Cuenta
                        </jet-responsive-nav-link>
                        <jet-responsive-nav-link :href="route('welcome')" :active="route().current('welcome')" v-else>
                            Inicio
                        </jet-responsive-nav-link>
                    </div>
            ...
            <div class="container mx-auto px-5">
                <footer class="flex flex-col justify-between space-y-1.5 text-gray-500 text-sm text-center sm:flex-row sm:space-y-0 sm:text-justify">
                    <p>Copyright © 2021 AlphaPlay. All rights reserved.</p>                    
                </footer>  
            </div>
                </section>
            </main>
